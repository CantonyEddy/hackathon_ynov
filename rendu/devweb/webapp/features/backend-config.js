class BackendConfigFeature {
  ...
  async check() {
    try {
      ...
      // Le proxy renvoie status:"ok" seulement si Ollama répond ET que le
      // modèle de prod est chargé. Sinon 503 + detail explicite.
      if (!response.ok || data.status !== 'ok') {
        this.statusElement.textContent = 'Hors ligne';
        this.modelElement.textContent = data.detail || data.backend || 'Backend injoignable';
        return;
      }
      this.statusElement.textContent = `Connecté • ${data.backend}`;
